...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        prev_state_nodes = self.s_levels[level] # Previous state level (previous to this level of actions)
        for action in self.all_actions:
            action_node = PgNode_a(action)
            if action_node.prenodes.issubset(prev_state_nodes):
                for s_node in prev_state_nodes:
            ...
            for effnode in a_node.effnodes:
                ...
                for existing_s_node in self.s_levels[level]:
                    if effnode == existing_s_node:
                        existing_s_node.parents.add(a_node)
                        a_node.children.add(existing_s_node)
                        is_unique = False
